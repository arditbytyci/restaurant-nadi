...
export const ForkKnife: React.FC<ForkKnifeProps> = ({
  ...
  return (
    <motion.svg
      ...
      className={`relative z-60 h-20 w-auto rounded-full md:h-22 lg:p-4 xl:h-30 ${className}`}
      animate={{ color }}
      transition={{ color: { duration: 0.08, ease: "linear" } }}
      style={{ willChange: "transform, color" }}
    >
      <motion.g
        animate={open ? { rotate: 40, x: 4, y: 2 } : { rotate: 0, x: 0, y: 0 }}
        transition={utensilTransition}
        style={{
